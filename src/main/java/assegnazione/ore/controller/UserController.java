...
public class UserController extends BaseService {
    ...
    @Autowired
    private final Element<User> userElement;
    ...
    @GetMapping("getLogin")
    public ResponseEntity<?> getUserInformations(@RequestParam String name, @RequestParam String lastName) {
        log.info("Enter into: " + getCurrentClassName() + " start method: " + getCurrentMethodName());
        try {
            log.info("Finish method: " + getCurrentMethodName());
            return ResponseEntity.ok().body(iUser.getUserInfo(name, lastName));
        } catch (RuntimeException e) {
            log.error("Error into: " + getCurrentClassName() + "method: " + getCurrentMethodName());
            return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
        } catch (Exception e) {
